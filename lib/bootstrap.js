...
/*
 * Helper to run the local task runner
 */
const localRunner = command => (argv, lando) => {
    const app = lando.getApp(argv._app.root);
    return app.init().then(() => {
        const task = _.find(app.tasks, {command});
        console.log(task);
    });
};
  ...
  _.forEach(_.get(config, 'tooling', {}), (task, command) => {
    tasks.push({
      ...
      run: (level === 'app')
          ...
          : (
              level === 'local'
                  ? engineRunner(config, command) //localRunner(command)
                  : engineRunner(config, command)
          ),
