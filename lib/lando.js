...
module.exports = class Lando {
  ...
  getApp(startFrom = process.cwd(), warn = true) {
    ...
    const landoFiles = _.flatten([
      utils.getLandoFiles(_.flatten([this.config.preLandoFiles, [this.config.landoFile]]), startFrom),
        _.first(_.flatten([
          utils.getLandoFiles(this.config.postLandoFiles, startFrom),
          utils.getLandoFiles(this.config.postLandoFiles, this.cli.defaultConfig().userConfRoot),
        ]))
    ]).filter(Boolean);
